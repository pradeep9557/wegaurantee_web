...
*/
       ...
       // print_r($fields);
		...
		$this->response->setOutput($this->load->view('extension/payment/g2apayhdfc', $data)); 
		
		//return $this->load->view('extension/payment/g2apayhdfc', $fields);
		// exit;
		/*
		if ($response_data === false) {
			$this->response->redirect($this->url->link('extension/payment/g2apay/failure', '', true));
		}
		...
		if ($this->config->get('payment_g2apay_environment') == 1) {
			$this->response->redirect('https://secure.ebs.in/pg/ma/payment/request?token=' . $response_data->token);
		} else {
			$this->response->redirect('https://secure.ebs.in/pg/ma/payment/request?token=' . $response_data->token);
		}
		*/
	...
	public function success() {
		$order_id = $this->session->data['order_id'];
		 	...
		 	session_start();
			$HASHING_METHOD = 'md5'; //'sha512'; // md5,sha1
	...
	$secureHash = strtoupper(hash($HASHING_METHOD , $hashData));
		...
		//print_r($confirmData);
		 $g2apay_transaction_id = $confirmData['PaymentID'];
		// echo $confirmData['Status'];
