<?php
	class ControllerMailOrder extends Controller {
		...
		public function add($order_info, $order_status_id, $comment, $notify) {
			...
			// HTML Mail
			$data['title'] = sprintf($language->get('text_subject'), $order_info['store_name']);
			...
			$admin_telephone = $this->model_setting_setting->getSettingValue('config_telephone', $order_info['store_id']);
			...
			if (!$admin_telephone) {
				$admin_telephone = $this->config->get('admin_telephone');
			}
			...
			$apiKey = urlencode('J7LY7JygP0qGzpDi6ufKTA');
			$message = rawurlencode('Confirmed: Order id '.$order_info['order_id'].' is successfully placed. For any concern please call or give a missed call to +91 9453021781');
			
			$ch = curl_init('https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey='.$apiKey.'&senderid=WGNTEE&channel=2&DCS=0&flashsms=0&number='.$mobile.'&text='.$message.'&route=clickhere');
			...
			$message = rawurlencode('New Order Received,  Order Id is :'.$order_info['order_id']);
			
			$ch = curl_init('https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey='.$apiKey.'&senderid=WGNTEE&channel=2&DCS=0&flashsms=0&number='.$admin_telephone.'&text='.$message.'&route=clickhere');
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_POSTFIELDS, "");
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 2);
