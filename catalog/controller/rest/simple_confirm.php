...
class ControllerRestSimpleConfirm extends RestController
{
    ...
    {
        error_log('saveOrderToDatabase entered');
        $this->checkPlugin();
        error_log('saveOrderToDatabase');
        $this->load->model('checkout/order');
        $this->load->model('account/order');
        error_log($this->session->data['order_id']);
        if (isset($this->session->data['order_id'])) {
            $order_status_id = 1;
            error_log('if entered');
            ...
            if (!isset($this->session->data['payment_method']) || empty($this->session->data['payment_method'])) {
                ...
                if (empty($status)) {
                    ...
                    error_log($this->session->data['order_id']);
                    error_log( $defaultStatus);
                    error_log( $this->session->data['comment']);
                    error_log( $this->session->data);
