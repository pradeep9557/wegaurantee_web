...
class ControllerExtensionModuleShoppingCartRestApi extends Controller {
    ...
    private function installHtaccess() {
        ...
        $newHtaccess = str_replace("RewriteCond %{REQUEST_FILENAME} !-f" , implode( "\n", array(
            ...
            '#######################################Forgot pass#####################################################',
            '#REST API voucher themes',
            'RewriteRule ^api/rest/account/fpassword index.php?route=rest/account/fpassword  [L]',
            '#######################################VOUCHERS#####################################################',
            '#REST API vouchers',
            ...
            '#######################################CHECKOUT For App SIMPLE############################################',
            '#REST API confirm and save order',
            'RewriteRule ^api/rest/checkoutforapp index.php?route=rest/simple_confirm/checkoutforapp  [L]',
